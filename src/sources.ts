...
import { basename, dirname, extname, isAbsolute, join, normalize, parse as parsePath, relative, resolve, sep } from 'node:path';
...
import type { PreparedAdmission as ContractPreparedAdmission, SourceKind as ContractSourceKind, SourceManifest as ContractSourceManifest } from './contracts.js';
...
const METADATA_HOSTNAMES: Record<string, true> = { metadata: true, 'metadata.google.internal': true, 'metadata.google.com': true, 'instance-data.ec2.internal': true, '100.100.100.200': true };
...
function ipv4Unsafe(address: string): boolean {
  ...
  return first === 0 || first === 10 || first === 127 || first >= 224 || (first === 169 && second === 254) || (first === 172 && second >= 16 && second <= 31) || (first === 192 && second === 168) || (first === 100 && second >= 64 && second <= 127) || (first === 192 && second === 0) || (first === 198 && (second === 18 || second === 19)) || (first === 198 && second === 51 && third === 100) || (first === 203 && second === 0 && third === 113) || (first === 169 && second === 254 && third === 169);
}
function ipv6Bytes(address: string): number[] | undefined {
  ...
  if (!left || !right || (halves.length === 1 && left.length !== 8) || (halves.length === 2 && left.length + right.length >= 8)) return undefined;
  const groups = [...left, ...(halves.length === 2 ? Array.from({ length: 8 - left.length - right.length }, () => 0) : []), ...right];
...
function unsafeAddress(address: string): boolean {
  ...
  if (loopback || bytes[0]! === 0xff || (bytes[0]! & 0xfe) === 0xfc || (bytes[0]! === 0xfe && (bytes[1]! & 0xc0) === 0x80)) return true;
...
async function safeAddressFor(url: URL): Promise<SafeAddress> {
  ...
  if (addresses.some(({ address }) => unsafeAddress(address))) throw new Error('source URL resolves to a private or special address');
...
function publicSourceUri(value: unknown): unknown {
  ...
  try { return sanitizedSourceUri(value); } catch { return undefined; }
}
...
function requestSource(url: URL, address: SafeAddress): Promise<SourceHttpResponse> {
  ...
  const baseOptions = {
    ...
    method: 'GET',
    agent: false,
    lookup: (_hostname: string, _options: object, callback: (error: NodeJS.ErrnoException | null, address: string, family: number) => void) => callback(null, address.address, address.family),
  };
  const onResponse = (response: IncomingMessage): void => {
    ...
    if (advertised !== undefined) {
      ...
      if (!Number.isSafeInteger(length) || length < 0) { response.destroy(); fail(new Error('source content length is invalid')); return; }
      if (length > MAX_SOURCE_BYTES) { response.destroy(); fail(new Error('source exceeds 100 MiB limit')); return; }
    }
    const chunks: Buffer[] = [];
    let total = 0;
    response.on('data', (chunk: Buffer | Uint8Array) => {
      ...
      if (total > MAX_SOURCE_BYTES) { response.destroy(); fail(new Error('source exceeds 100 MiB limit')); return; }
      chunks.push(bytes);
    });
    ...
    response.once('end', () => {
      if (settled) return;
      settled = true;
      resolveRequest({ status, bytes: Buffer.concat(chunks, total), mediaType: response.headers['content-type']?.toString() });
    });
  };
  request = url.protocol === 'https:' ? httpsRequest({ ...baseOptions, servername: baseOptions.hostname }, onResponse) : httpRequest(baseOptions, onResponse);
...
function validRelativePath(value: string): string {
  if (!value || /[\u0000-\u001f\u007f]/u.test(value) || value.includes('\\') || isAbsolute(value)) throw new Error('invalid relative path');
...
function workArtifactRelative(paths: VaultPathsLike, target: string): string {
  ...
  if (!workRelative || workRelative === '..' || workRelative.startsWith('../') || isAbsolute(workRelative)) throw new Error('prepared path escapes work');
...
const SAFE_REPOSITORY_GIT_ENV = { GIT_CONFIG_NOSYSTEM: '1', GIT_CONFIG_SYSTEM: '/dev/null', GIT_CONFIG_GLOBAL: '/dev/null', GIT_OPTIONAL_LOCKS: '0' } as const;
async function repositoryGit(root: string, command: 'ls-files' | 'rev-parse', maxOutputBytes: number): Promise<Awaited<ReturnType<typeof runChild>>> {
  const args = command === 'ls-files' ? [...SAFE_REPOSITORY_GIT_ARGS, 'ls-files', '--cached', '--others', '--exclude-standard', '-z'] : [...SAFE_REPOSITORY_GIT_ARGS, 'rev-parse', 'HEAD'];
  return runChild('git', ['-C', root, ...args], { cwd: root, timeoutMs: 120_000, maxOutputBytes, env: SAFE_REPOSITORY_GIT_ENV });
}
async function repositoryRevision(root: string): Promise<string> {
  const result = await repositoryGit(root, 'rev-parse', 1024);
  if (result.timedOut || result.code !== 0) throw new Error(`git revision lookup failed (${result.code ?? result.signal ?? 'unknown'})`);
...
async function repositoryFiles(root: string): Promise<FileSnapshot[]> {
  ...
  if (result.timedOut || result.code !== 0) throw new Error(`git file listing failed (${result.code ?? result.signal ?? 'unknown'})`);
  if (Buffer.byteLength(result.stdout, 'utf8') >= MAX_SOURCE_BYTES) throw new Error('repository file listing exceeds 100 MiB limit');
  const files: FileSnapshot[] = [];
...
async function stagedMetadata(path: string): Promise<StageMetadata | undefined> {
  ...
  try {
    ...
    if (metadata.kind === 'repository' ? !payloadStat.isDirectory() : !payloadStat.isFile()) throw new Error(`staged source payload must be a regular ${metadata.kind === 'repository' ? 'directory' : 'file'}`);
    return metadata;
  } catch (error) {
...
async function snapshotPath(path: string, relativePath: string, kind?: SourceKind, revision?: string, metadata?: StageMetadata): Promise<TreeSnapshot> {
  ...
  const files = stat.isDirectory() ? kind === 'repository' && !metadata ? await repositoryFiles(path) : await walkFiles(path) : [{ path: basename(path), size: 0, digest: '', bytes: Buffer.alloc(0) }];
  if (!stat.isDirectory()) {
    const first = files[0];
...
function parseManifestValue(raw: unknown): SourceManifest {
  ...
  if (record.stagedMetadata !== undefined) {
    if (record.stagedMetadata === null || typeof record.stagedMetadata !== 'object' || Array.isArray(record.stagedMetadata)) throw new Error('invalid source manifest metadata');
...
function manifestInteger(value: unknown, key: string): number {
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) throw new Error(`invalid source manifest ${key}`);
  return value;
}
...
async function verifyRetainedPacket(packet: string, expected: { sourceId: string; originalDigest: string }): Promise<SourceManifest> {
  ...
  if (names.length !== allowed.length || names.some((name, index) => name !== allowed[index])) throw new Error('source packet contains unexpected artifacts');
  ...
  const expectedFiles = manifest.files.map((file, index) => {
    ...
    const relativePath = record.relativePath;
    if (typeof relativePath !== 'string' || typeof record.path !== 'string' || record.path !== relativePath) throw new Error(`invalid source manifest file ${index}`);
    ...
    if (manifestInteger(record.bytes, `files[${index}].bytes`) !== byteLength) throw new Error(`source manifest file length mismatch: ${relativePath}`);
    return { relativePath: normalizedPath, byteLength, digest: manifestDigest(record.digest, `files[${index}].digest`) };
  });
  const actualFiles = originalFiles.map((file) => ({ relativePath: file.path, byteLength: file.size, digest: file.digest }));
  ...
  if (manifestInteger(manifest.originalBytes, 'originalBytes') !== originalBytes || manifestInteger(manifest.originalByteLength, 'originalByteLength') !== originalBytes) throw new Error('retained source original length mismatch');
  ...
  if (manifestInteger(manifest.extractionBytes, 'extractionBytes') !== extractedLength || extractedLength !== extracted.byteLength) throw new Error('retained source extraction length mismatch');
  const extractedDigest = manifestDigest(manifest.extractedDigest, 'extractedDigest');
  if (manifestDigest(manifest.extractionDigest, 'extractionDigest') !== extractedDigest || digestBytes(extracted) !== extractedDigest) throw new Error('retained source extraction digest mismatch');
  ...
  if (!manifest.chunks.length || chunkNames.length !== manifest.chunks.length) throw new Error('retained source chunks are incomplete');
  ...
  for (const [index, chunk] of manifest.chunks.entries()) {
    ...
    if (manifestInteger(record.index, `chunks[${index}].index`) !== index || manifestInteger(record.ordinal, `chunks[${index}].ordinal`) !== index || record.sourceId !== expected.sourceId || record.chunkId !== `${expected.sourceId}:${index}` || record.relativePath !== 'extracted.md') throw new Error('retained source chunk metadata mismatch');
    ...
    if (startAtom !== atomStart || endAtom !== atomEnd || startAtom !== nextAtom || endAtom <= startAtom || endAtom > atoms.length || startByte !== nextByte || endByte < startByte || endByte - startByte !== body.byteLength || endByte - startByte !== manifestInteger(record.byteLength, `chunks[${index}].byteLength`)) throw new Error('retained source chunk coverage is invalid');
    const firstAtom = atoms[startAtom];
    const lastAtom = atoms[endAtom - 1];
    if (!firstAtom || !lastAtom || firstAtom.startByte !== startByte || lastAtom.endByte !== endByte || !body.equals(extracted.subarray(startByte, endByte)) || digestBytes(body) !== manifestDigest(record.digest, `chunks[${index}].digest`)) throw new Error('retained source chunk digest mismatch');
    chunkBodies.push(body);
    nextAtom = endAtom;
    nextByte = endByte;
  }
  if (nextAtom !== atoms.length || nextByte !== extracted.byteLength || !Buffer.concat(chunkBodies).equals(extracted)) throw new Error('retained source chunk reconstruction is incomplete');
  return manifest;
}

function objectRecord(value: unknown, label: string): Record<string, unknown> {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new Error(`invalid prepared metadata ${label}`);
  return value as Record<string, unknown>;
}
function integerField(record: Record<string, unknown>, key: string, minimum = 0): number {
  const value = record[key];
  if (typeof value !== 'number' || !Number.isInteger(value) || value < minimum) throw new Error(`invalid prepared metadata ${key}`);
  return value;
}
function identityField(value: unknown, label: string): PhysicalIdentity {
  const record = objectRecord(value, label);
  const mode = record.mode;
  if (typeof record.device !== 'string' || typeof record.inode !== 'string' || typeof mode !== 'number' || !Number.isInteger(mode) || typeof record.size !== 'number' || !Number.isInteger(record.size) || record.size < 0 || typeof record.mtimeNs !== 'string') throw new Error(`invalid prepared metadata ${label}`);
...
function parsePreparedMetadata(raw: unknown): PersistedPreparedAdmission {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[45][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(preparedId)) throw new Error('invalid prepared metadata preparedId');
  ...
  if (new Set(files.map((file) => file.relativePath)).size !== files.length) throw new Error('invalid prepared metadata duplicate file');
  ...
  const atoms = atomsValue.map((value) => {
    ...
    const byteLength = integerField(item, 'byteLength');
    if (endByte < startByte || endByte - startByte !== byteLength) throw new Error('invalid prepared metadata atom bounds');
  ...
  const attachments = attachmentsValue.map((value) => {
    ...
    return { path: validRelativePath(requiredString(item, 'path')), byteLength: integerField(item, 'byteLength'), digest: requiredString(item, 'digest') };
  });
  if (new Set(attachments.map((attachment) => attachment.path)).size !== attachments.length) throw new Error('invalid prepared metadata duplicate attachment');
...
export class SourceService {
  ...
  private preparedRoot(preparedId: string): string {
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[45][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(preparedId)) throw new Error('invalid prepared id');
    return safeRelativePath(this.work(), `admission-${preparedId}`);
  }
  private preparedMetadataPath(preparedId: string): string { return join(this.preparedRoot(preparedId), '.pi-scholar-prepared.json') }
  ...
  private async revalidateClaim(claim: SourceClaim): Promise<SourceClaim> {
    ...
    if (current.claimId !== claim.claimId || current.snapshot.digest !== claim.snapshot.digest || !sameIdentity(current.entry.identity, claim.entry.identity) || !sameIdentity(current.snapshot.identity, claim.snapshot.identity)) throw new Error('source claim is stale');
    return current;
  }
  private assertPreparedClaim(prepared: PersistedPreparedAdmission, claim: SourceClaim, digest: string): void {
    if (prepared.claimId !== claim.claimId || prepared.entryRelativePath !== claim.entry.relativePath || prepared.digest !== digest || claim.snapshot.digest !== digest || !sameIdentity(prepared.entryIdentity, claim.entry.identity) || !sameIdentity(prepared.snapshotIdentity, claim.snapshot.identity) || prepared.snapshotBytes !== claim.snapshot.bytes) throw new Error('prepared source claim is stale');
    const expectedFiles = claim.snapshot.files.map((file) => ({ relativePath: file.path, byteLength: file.size, digest: file.digest }));
  ...
  async stage(request: SourceStageRequest): Promise<{ relativePath: string; absolutePath: string; kind: SourceKind; metadata?: StageMetadata }> {
    ...
    const forms = [request.url !== undefined, request.text !== undefined, request.bytes !== undefined, request.path !== undefined, request.filePath !== undefined].filter(Boolean).length;
    ...
    if (request.url !== undefined) {
      ...
      const metadata: StageMetadata = { version: 1, requestedKind: request.kind ?? 'url', kind: 'url', displayName, originalName, sourceUri: provenanceUrl(parsed), mediaType, payload: 'payload' };
      return this.stageEnvelope(metadata, fetched.bytes);
    }
    if (request.text !== undefined) {
      if (request.kind !== undefined && request.kind !== 'text' && request.kind !== 'pasted') throw new Error('text source kind must be text or pasted');
      ...
      const metadata: StageMetadata = { version: 1, requestedKind, kind: 'text', displayName, originalName, mediaType: request.mediaType, payload: 'payload' };
    ...
    if (request.bytes !== undefined) {
      ...
      const displayName = request.displayName ?? originalName;
      if (/[\u0000-\u001f\u007f]/u.test(displayName) || (request.mediaType !== undefined && /[\u0000-\u001f\u007f]/u.test(request.mediaType))) throw new Error('invalid staged source metadata');
      const metadata: StageMetadata = { version: 1, requestedKind: 'upload', kind, displayName, originalName, mediaType: request.mediaType, payload: 'payload' };
      return this.stageEnvelope(metadata, bytes);
    }
    const input = request.path ?? request.filePath;
    if (!input) throw new Error('source input is required');
    if (request.kind !== undefined && !SOURCE_KINDS.includes(request.kind as SourceKind)) throw new Error('invalid source kind');
    ...
    const metadata = repository ? (() => {
      ...
      const displayName = request.displayName ?? originalName;
      if (/[\u0000-\u001f\u007f]/u.test(displayName) || (request.mediaType !== undefined && /[\u0000-\u001f\u007f]/u.test(request.mediaType))) throw new Error('invalid staged source metadata');
      return { version: 1 as const, requestedKind: (request.kind ?? kind) as InputKind, kind, displayName, originalName, mediaType: request.mediaType, repositoryRevision: revision, payload: 'payload' as const };
    })() : undefined;
    ...
    if (source !== target) {
      ...
      try {
        ...
        if (!targetExisted) {
          try { await fs.rm(target, { recursive: true, force: true }); } catch { /* Preserve the original staging failure. */ }
        }
        throw error;
      }
    }
    return { relativePath: relative(this.inbox(), target).replaceAll('\\', '/'), absolutePath: target, kind, ...(metadata ? { metadata } : {}) };
  }
  ...
  async prepareClaim(input: SourceClaim | InboxEntry): Promise<PreparedAdmission> {
    ...
    try {
      ...
      if (useDocling) {
        ...
        const converted = typeof this.adapters.docling === 'function' ? await this.adapters.docling({ claim, originalPath, kind: claim.snapshot.kind, mediaType }) : await this.adapters.docling.convert({ claim, originalPath, kind: claim.snapshot.kind, mediaType });
      ...
      const atoms = atomizeExtraction(extracted).map((atom) => ({ index: atom.index, startByte: atom.startByte, endByte: atom.endByte, byteLength: atom.endByte - atom.startByte }));
      const prepared: PreparedAdmission = {
        ...
        files: claim.snapshot.files.map((file) => ({ relativePath: file.path, byteLength: file.size, digest: file.digest })),
        atoms,
      };
      const attachmentRecords = attachmentSnapshots.map(({ path, size, digest }) => ({ path, byteLength: size, digest })).sort((left, right) => left.path.localeCompare(right.path));
  ...
  async cleanupPrepared(preparedId: string): Promise<void> {
    ...
    if (!withinWork || withinWork === '..' || withinWork.startsWith('../') || isAbsolute(withinWork)) throw new Error('prepared path escapes work');
    await fs.rm(root, { recursive: true, force: true });
    this.retainedPrepared.delete(preparedId);
  }
  private async publishPacket(manifest: SourceManifest, temporary: string, packet: string, claim: SourceClaim): Promise<{ manifest: SourceManifest; removedInbox: boolean }> {
    ...
    try {
      ...
      try {
        ...
        const packetExists = await lstatNoFollow(packet).then((stat) => stat.isDirectory(), (error) => {
      ...
      publishedManifest = await verifyRetainedPacket(packet, { sourceId: manifest.sourceId, originalDigest: claim.snapshot.digest });
      this.recordSource(publishedManifest, packet);
    } catch (error) {
      if (createdPacket && createdPacketIdentity) {
        try {
          const current = await lstatNoFollow(packet);
          if (sameIdentity(statIdentity(current), createdPacketIdentity)) await fs.rm(packet, { recursive: true, force: false });
        } catch { /* Preserve the database or publication failure. */ }
      }
      throw error;
    } finally {
      if (temporaryPresent) {
        try { await fs.rm(temporary, { recursive: true, force: true }); } catch { /* Best-effort cleanup after a failed publication. */ }
      }
    }
    return { manifest: publishedManifest, removedInbox: await this.removeInboxAfterAdmission(claim) };
  }
  async publishPreparedClaim(input: { prepared: PreparedAdmission; preparedId: string; claimId: string; digest: string; endpoints?: readonly number[] }): Promise<AdmissionResult> {
    const completed = this.completedPrepared.get(input.preparedId);
    if (completed) {
      if (completed.prepared !== input.prepared || completed.result.claim.claimId !== input.claimId || completed.result.claim.snapshot.digest !== input.digest) throw new Error('prepared admission identity mismatch');
      return completed.result;
    }
    const retained = this.retainedPrepared.get(input.preparedId);
    if (!retained || retained.prepared !== input.prepared) throw new Error('prepared admission is not the retained host record');
    const prepared = await this.loadPrepared(input.preparedId);
    if (canonical(prepared) !== retained.seal || prepared.claimId !== input.claimId || prepared.digest !== input.digest) throw new Error('prepared claim identity mismatch');
    ...
    if (resolve(originalRoot) !== resolve(join(preparedRoot, 'original')) || resolve(extractedPath) !== resolve(join(preparedRoot, 'extracted.md'))) throw new Error('prepared artifact path mismatch');
    ...
    const preparedFileRecords = preparedFiles.map((file) => ({ relativePath: file.path, byteLength: file.size, digest: file.digest }));
    if (canonical(preparedFileRecords) !== canonical(prepared.files) || treeDigest(preparedFiles, prepared.snapshotIdentity, prepared.revision) !== prepared.digest) throw new Error('prepared snapshot digest mismatch');
    const extracted = await readNoFollow(extractedPath);
    if (extracted.byteLength !== prepared.extractedByteLength || digestBytes(extracted) !== prepared.extractedDigest) throw new Error('prepared extraction digest mismatch');
    const actualAtoms = atomizeExtraction(extracted).map((atom) => ({ index: atom.index, startByte: atom.startByte, endByte: atom.endByte, byteLength: atom.endByte - atom.startByte }));
    ...
    const preparedAttachmentRecords = preparedAttachmentFiles.map((file) => ({ path: file.path, byteLength: file.size, digest: file.digest }));
    if (canonical(preparedAttachmentRecords) !== canonical(prepared.attachments)) throw new Error('prepared attachment digest mismatch');
    ...
    try {
      ...
      try { await fs.rm(temporary, { recursive: true, force: true }); } catch { /* Best-effort cleanup after packet construction failure. */ }
      throw error;
    }
  ...
  private async revision(path: string): Promise<string | undefined> {
    ...
    try { return await repositoryRevision(path); } catch { return undefined }
  }
  ...
  async admitClaim(claim: SourceClaim, options: { endpoints?: Array<number | ChunkPlanEndpoint>; mediaType?: string; originalName?: string; url?: string } = {}): Promise<AdmissionResult> {
    ...
    const published = await this.publishPacket(manifest, temporary, packet, claim);
    return { sourceId: published.manifest.sourceId, manifest: published.manifest, packetPath: packet, removedInbox: published.removedInbox, claim };
  }
  ...
  private recordSource(manifest: SourceManifest, packet: string): void {
    transaction(this.db, () => {
      ...
      if (!existing) dbRun(this.db, 'INSERT INTO sources (source_id, kind, status, display_name, original_name, source_uri, media_type, repository_revision, captured_at, digest, manifest_path, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', values);
        ...
        if (String(existing.digest ?? '') !== manifest.originalDigest) throw new Error('source identity conflicts with existing database record');
        const now = new Date().toISOString();
        dbRun(this.db, 'UPDATE sources SET kind = ?, status = ?, display_name = ?, original_name = ?, source_uri = ?, media_type = ?, repository_revision = ?, captured_at = ?, digest = ?, manifest_path = ?, error_code = NULL, error_message = NULL, updated_at = ? WHERE source_id = ?', [manifest.kind, 'published', manifest.displayName, manifest.originalName, manifest.sourceUri ?? null, manifest.mediaType ?? null, manifest.repositoryRevision ?? null, manifest.capturedAt, manifest.originalDigest, packet, now, manifest.sourceId]);
      }
  ...
  private refreshDependencies(): void {
    transaction(this.db, () => {
      dbRun(this.db, "DELETE FROM source_dependencies WHERE relation IN ('claim', 'question') OR (relation = 'citation' AND page_id IS NOT NULL)");
      const sources = dbAll<{ source_id: string }>(this.db, "SELECT source_id FROM sources WHERE status != 'removed' ORDER BY source_id");
      const chunks = dbAll<{ source_id: string; chunk_id: string }>(this.db, 'SELECT source_id, chunk_id FROM source_chunks ORDER BY source_id, ordinal');
      ...
      const pages = dbAll<Row>(this.db, "SELECT page_id, relative_path FROM pages WHERE status != 'retired' ORDER BY relative_path, page_id");
      ...
      for (const page of pages) {
        ...
        let body: string;
        try { body = readFileSync(absolute, 'utf8'); } catch (error) {
      ...
      for (const question of questions) {
        ...
        if (pageId && refs.some((reference) => reference.split('#', 1)[0] === pageId)) {
          for (const dependency of dbAll<Row>(this.db, "SELECT source_id, page_id, chunk_id FROM source_dependencies WHERE relation = 'citation' AND page_id = ? ORDER BY source_id, chunk_id", [pageId])) {
            dbRun(this.db, 'INSERT OR IGNORE INTO source_dependencies (source_id, page_id, chunk_id, relation) VALUES (?, ?, ?, ?)', [dependency.source_id, dependency.page_id, dependency.chunk_id ?? null, 'question']);
          }
        }
        for (const source of sources) {
          const sourceId = String(source.source_id);
          if (refs.includes(sourceId) && pageId) dbRun(this.db, 'INSERT OR IGNORE INTO source_dependencies (source_id, page_id, chunk_id, relation) VALUES (?, ?, ?, ?)', [sourceId, pageId, null, 'question']);
          for (const chunkId of chunksBySource.get(sourceId) ?? []) if (refs.includes(chunkId)) dbRun(this.db, 'INSERT OR IGNORE INTO source_dependencies (source_id, page_id, chunk_id, relation) VALUES (?, ?, ?, ?)', [sourceId, pageId ?? null, chunkId, 'question']);
        }
      }
    });
  }
  private currentDependents(sourceId: string): Array<Record<string, unknown>> {
    const rows: Array<Record<string, unknown>> = dbAll<Row>(this.db, "SELECT sd.*, p.relative_path AS artifact_path, p.digest AS artifact_digest FROM source_dependencies sd LEFT JOIN pages p ON p.page_id = sd.page_id WHERE sd.source_id = ? AND sd.relation = 'citation' AND (p.page_id IS NULL OR p.status != 'retired') ORDER BY sd.page_id, sd.chunk_id, sd.relation", [sourceId]).map((row) => ({ ...row, ...(row.artifact_digest ? { digest: row.artifact_digest } : {}), kind: 'citation' }));
    const cards = dbAll<Row>(this.db, "SELECT DISTINCT sd.source_id, sd.page_id, sd.chunk_id, sd.relation, cb.card_id, p.relative_path AS artifact_path, p.digest AS artifact_digest FROM source_dependencies sd JOIN card_bindings cb ON cb.page_id = sd.page_id AND cb.active = 1 JOIN review_cards rc ON rc.card_id = cb.card_id AND rc.status = 'active' LEFT JOIN pages p ON p.page_id = sd.page_id WHERE sd.source_id = ? AND sd.relation = 'citation' AND p.status != 'retired' ORDER BY cb.card_id, sd.page_id, sd.chunk_id", [sourceId]);
    rows.push(...cards.map((row) => ({ ...row, relation: 'claim', ...(row.artifact_digest ? { digest: row.artifact_digest } : {}), kind: 'card' })));
    const chunks = dbAll<{ chunk_id: string }>(this.db, 'SELECT chunk_id FROM source_chunks WHERE source_id = ? ORDER BY chunk_id', [sourceId]).map((row) => row.chunk_id);
    const questions = dbAll<Row>(this.db, "SELECT q.quiz_id, qq.question_id, qq.source_refs_json, qc.card_id, cb.page_id, q.date FROM quizzes q JOIN quiz_questions qq ON qq.quiz_id = q.quiz_id JOIN question_cards qc ON qc.question_id = qq.question_id JOIN card_bindings cb ON cb.card_id = qc.card_id AND cb.active = 1 JOIN review_cards rc ON rc.card_id = cb.card_id AND rc.status = 'active' JOIN pages p ON p.page_id = cb.page_id AND p.status != 'retired' WHERE q.status = 'open' ORDER BY q.quiz_id, qq.question_id, qc.card_id, cb.page_id");
    for (const question of questions) {
      const refs = this.sourceRefs(question.source_refs_json);
      if (!refs.length) continue;
      const pageId = typeof question.page_id === 'string' ? question.page_id : undefined;
      if (pageId && refs.some((reference) => reference.split('#', 1)[0] === pageId)) {
        for (const dependency of dbAll<Row>(this.db, "SELECT source_id, page_id, chunk_id FROM source_dependencies WHERE source_id = ? AND relation = 'citation' AND page_id = ?", [sourceId, pageId])) rows.push({ ...dependency, quiz_id: question.quiz_id, date: question.date, question_id: question.question_id, card_id: question.card_id, relation: 'question', kind: 'quiz' });
      }
      if (refs.includes(sourceId)) rows.push({ source_id: sourceId, page_id: pageId ?? null, chunk_id: null, quiz_id: question.quiz_id, date: question.date, question_id: question.question_id, card_id: question.card_id, relation: 'question', kind: 'quiz' });
      for (const chunkId of chunks) if (refs.includes(chunkId)) rows.push({ source_id: sourceId, page_id: pageId ?? null, chunk_id: chunkId, quiz_id: question.quiz_id, date: question.date, question_id: question.question_id, card_id: question.card_id, relation: 'question', kind: 'quiz' });
    }
    return rows;
  }
  private removalLocations(sourceId: string, source: Row): { digest: string; packetPath: string; quarantineRoot: string; quarantine: string } {
  ...
  private removalCleanupPending(sourceId: string, cause: unknown): Error {
    return Object.assign(new Error(`source ${sourceId} was removed but packet cleanup is pending; retry removal`), { code: 'SOURCE_REMOVAL_CLEANUP_PENDING', details: { applied: true, retryable: true }, cause });
  }
  private async reconcileRemovalState(sourceId: string, source: Row, locations: { digest: string; packetPath: string; quarantineRoot: string; quarantine: string }): Promise<SourceRemovalResult | undefined> {
    ...
    if (String(source.status ?? '') === 'removed') {
      ...
      return { sourceId, packetPath: locations.packetPath, currentDigest: locations.digest, dependents, confirmationId: digestBytes(Buffer.from(canonical({ sourceId, currentDigest: locations.digest, dependents }))), removed: true };
    }
    if (!originalPresent && quarantinePresent) {
      ...
      if (!await this.validRemovalPacket(locations.packetPath, sourceId, locations.digest)) throw new Error('source packet restore failed');
    }
  ...
  removalPreview(sourceId: string): SourceRemovalPreview {
    ...
    if (manifest.sourceId !== sourceId || manifest.originalDigest !== locations.digest) throw new Error('source packet identity mismatch');
