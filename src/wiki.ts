...
export interface QmdAdapter { search(query: string, options?: { collection: string; scope: 'wiki/**/*.md'; limit?: number }): Promise<unknown> | unknown; index?: () => Promise<void> | void }
...
export interface WikiPreparedUpdate extends WikiCreateResult { expectedDigest: string }
...
function validateMarkdownLinks(root: string, pagePath: string, body: string): void {
  for (const raw of linksFromMarkdown(body)) {
    ...
    let decoded: string;
    try { decoded = decodeURIComponent(target); } catch { throw new Error('invalid Markdown link encoding'); }
    if (!decoded || decoded.startsWith('/') || decoded.startsWith('//') || decoded.includes('\\') || /^[A-Za-z][A-Za-z0-9+.-]*:/u.test(decoded) || /^[A-Za-z]:/u.test(decoded)) {
...
export class WikiService {
  ...
  private async optionalBytes(path: string): Promise<Buffer | undefined> {
    try { return await fs.readFile(path); } catch (error) {
  ...
  private async authoredPage(page: WikiPage): Promise<(WikiPage & { content: string }) | undefined> {
    ...
    try {
      ...
      if (!snapshot || currentDigest !== snapshot.digest || currentDigest !== page.digest || current.status !== 'active') return undefined;
      return current;
    } catch {
      return undefined;
    }
  ...
  async prepareUpdate(pageId: string, input: { body?: string; title?: string; quizWorthiness?: WikiPage['quizWorthiness']; expectedDigest?: string; path?: string }, updatedAt = now()): Promise<WikiPreparedUpdate> {
    const row = this.catalog(pageId);
    ...
    const content = serializePage(pageId, input.title ?? parsed.fields.title ?? page.title, body, input.quizWorthiness ?? page.quizWorthiness, parsed.fields.created ?? updatedAt, updatedAt);
  ...
  async update(pageId: string, input: { body?: string; title?: string; quizWorthiness?: WikiPage['quizWorthiness']; expectedDigest?: string; path?: string }, prepared?: WikiPreparedUpdate): Promise<WikiCreateResult> {
    ...
    const location = normalizePagePath(this.paths, input.path ?? page.relativePath);
    if (location.relativePath !== page.relativePath || next.page.pageId !== pageId || next.page.revision !== page.revision + 1) throw new Error('prepared wiki update is stale');
    ...
    const snapshotPath = this.snapshotPath(page);
    const priorSnapshotRow = dbGet<SnapshotRow>(this.db, 'SELECT * FROM authored_snapshots WHERE relative_path = ?', [page.relativePath]);
    ...
    const rollback = async (): Promise<void> => {
      ...
      await attempt(() => {
        transaction(this.db, () => {
          dbRun(this.db, 'UPDATE pages SET relative_path = ?, title = ?, digest = ?, revision = ?, status = ?, quiz_worthiness = ?, updated_at = ? WHERE page_id = ?', [priorPageRow.relative_path, priorPageRow.title, priorPageRow.digest, priorPageRow.revision, priorPageRow.status, priorPageRow.quiz_worthiness, priorPageRow.updated_at, pageId]);
          dbRun(this.db, 'DELETE FROM authored_snapshots WHERE relative_path = ?', [page.relativePath]);
          if (priorSnapshotRow) dbRun(this.db, 'INSERT OR REPLACE INTO authored_snapshots (relative_path, digest, revision, captured_at, commit_id) VALUES (?, ?, ?, ?, ?)', [priorSnapshotRow.relative_path, priorSnapshotRow.digest, priorSnapshotRow.revision, priorSnapshotRow.captured_at, priorSnapshotRow.commit_id ?? null]);
        });
      });
      await attempt(() => this.restoreOptional(indexPath, priorIndex));
      await attempt(() => this.restoreOptional(logPath, priorLog));
  ...
  private qmdPath(value: unknown): string {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new Error('qmd returned malformed search result');
    const record = value as Record<string, unknown>;
    const relativePath = (path: string): string => {
      if (!path || /[\u0000-\u001f\u007f]/u.test(path) || path.includes('\\') || path.startsWith('/') || path.startsWith('//') || /^[A-Za-z]:/u.test(path) || /^[A-Za-z][A-Za-z0-9+.-]*:/u.test(path)) throw new Error('qmd returned an unsafe wiki path');
      const normalized = normalize(path).replaceAll('\\', '/');
      const candidate = normalized.startsWith('wiki/') ? normalized.slice('wiki/'.length) : normalized;
      if (!candidate || candidate === '.' || !candidate.endsWith('.md') || path.split('/').some((part) => part === '..') || candidate.split('/').some((part) => part === '.snapshots' || part === '.pi-scholar')) throw new Error('qmd returned an unsafe wiki path');
      return candidate;
    };
    ...
    if (rawPath.split('/').some((part) => {
      try { const decoded = decodeURIComponent(part); return decoded === '..' || decoded === '.'; } catch { return false; }
    })) throw new Error('qmd returned an unsafe wiki URI');
    let parsed: URL;
    try { parsed = new URL(record.file); } catch { throw new Error('qmd returned a malformed wiki URI'); }
    const collection = qmdCollectionName(this.paths.vaultId);
    if (parsed.protocol !== 'qmd:' || parsed.host !== collection || parsed.username || parsed.password || parsed.port || parsed.search || parsed.hash || !parsed.pathname.startsWith('/') || /%(?:2f|5c)/iu.test(parsed.pathname)) throw new Error('qmd returned an unsafe wiki URI');
    let decoded: string;
    try { decoded = decodeURIComponent(parsed.pathname.slice(1)); } catch { throw new Error('qmd returned a malformed wiki URI'); }
  ...
  async inspectDrift(pageId: string): Promise<DriftReport> {
    ...
    return { page: currentPage, drifted, authoredDigest, currentDigest, diff: contentDrifted ? simpleDiff(snapshot.content, page.content) : '', choices: ['record-issue', 'restore'] };
  }
  async resolveDrift(pageId: string, choice: DriftResolution): Promise<{ page: WikiPage; issue?: WikiIssue; restored: true }> {
    ...
    if (report.authoredDigest === report.currentDigest) throw new Error('semantic drift requires maintenance correction');
    const snapshot = this.authored(pageId);
    if (!snapshot) throw new Error('product-authored snapshot is unavailable');
    const priorPageRow = this.catalog(pageId);
    if (!priorPageRow) throw new Error('page not found');
    ...
    const priorSnapshotRow = dbGet<SnapshotRow>(this.db, 'SELECT * FROM authored_snapshots WHERE relative_path = ?', [report.page.relativePath]);
    ...
    const issue = choice === 'record-issue'
      ? {
          issueId: randomUUID(),
          pageId,
    ...
    try {
      ...
      try { await restore(); } catch (rollbackError) {
        throw new Error(`wiki drift resolution failed and rollback failed: ${error instanceof Error ? error.message : String(error)}; ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`, { cause: rollbackError });
      }
      throw error;
    }
  ...
  async resolveIssueAfterCorrection(issueId: string, resolution: string): Promise<WikiIssue> {
    ...
    const updatedAt = now();
    transaction(this.db, () => dbRun(this.db, 'UPDATE wiki_issues SET status = ?, resolution = ?, updated_at = ? WHERE issue_id = ?', ['resolved', resolution, updatedAt, issueId]));
    const updated = dbGet<IssueRow>(this.db, 'SELECT * FROM wiki_issues WHERE issue_id = ?', [issueId]);
    if (!updated) throw new Error('issue disappeared after update');
    return rowToIssue(updated);
  }
  async refreshProjections(write = true): Promise<{ indexPath: string; logPath: string; backlinks: Record<string, string[]>; lint: string[] }> {
