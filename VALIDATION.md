...
2. Stage a representative source with `/scholar-add` or the supported inbox path. Exercise streamed or disk-backed capture with a source large enough to use those paths; verify there is no fixed product source-size cutoff and that the product does not promise predictive free-space preflight. Preserve the original bytes; OS capacity failures are covered by the destructive disposable-vault check below.
...
- Perform a destructive OS `ENOSPC` check during URL, multipart, or source publication in a disposable vault (never a real user vault). Confirm accepted bytes and provenance remain intact or transactionally uncommitted, temporary work artifacts are cleaned, and SQLite/Git plus `pi-scholar doctor` remain recoverable and healthy after cleanup and retry.
