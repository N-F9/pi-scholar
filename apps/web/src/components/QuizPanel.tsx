import type { PublicQuizQuestionRecord, QuizAnswerInput, QuizGradeRecord, QuizQuestionResultRecord, QuizReadingRecord } from "../../../../src/contracts";
...
function readingTarget(reading: QuizReadingRecord): string {
  ...
  try {
    ...
    if (supplied.origin === "https://scholar.invalid") {
      if (supplied.pathname === "/notes") supplied.searchParams.forEach((value, key) => search.append(key, value));
...
export function ReadOnlyQuestions({ questions, answers }: { questions: readonly PublicQuizQuestionRecord[]; answers: readonly QuizAnswerInput[] }) {
  const byQuestion = new Map(answers.map((item) => [item.questionId, item.answer]));
  return (
    <ol className="grid gap-5">
      {[...questions].sort((a, b) => a.ordinal - b.ordinal).map((question) => (
            ...
            {question.kind === "multiple-choice" && question.choices?.length ? (
              <ul className="mt-4 grid gap-2">
                {question.choices.map((choice) => {
                  ...
                  return <li className={selected ? "rounded-md border border-accent bg-accent/10 px-3 py-2 font-bold" : "rounded-md border border-line px-3 py-2 text-muted"} key={choice}>{choice}{selected ? " — selected" : ""}</li>;
                })}
              </ul>
            ) : (
...
export function QuizResults({ questions, questionResults, grades, readings }: {
  ...
  const questionIds = [...new Set([...questionResults.map((result) => result.questionId), ...grades.map((grade) => grade.questionId)])];

  return (
    <section className="space-y-5" aria-labelledby="results-heading">
      <div>
        <p className="eyebrow">Settled review</p>
        <h2 className="mt-2 font-serif text-3xl font-semibold" id="results-heading">Results</h2>
      </div>
          ...
          <Card className="shadow-none" key={result?.resultId ?? questionId}>
            ...
            <ul className="mt-2 grid gap-2">
              {cardGrades.map((grade) => (
                <li className="flex flex-wrap items-start justify-between gap-3 rounded-md border border-line bg-canvas p-3" key={grade.gradeId}>
                  <div>
            ...
            {readings.map((reading) => {
              ...
              return (
                <li key={`${reading.pageId}:${reading.heading ?? ""}`}>
                  <Link className="flex min-h-11 items-center justify-between rounded-md border border-line bg-paper px-3 py-2 font-bold hover:border-ink" to={target}>
                    <span>{reading.path}{reading.heading ? ` — ${reading.heading}` : ""}</span>
