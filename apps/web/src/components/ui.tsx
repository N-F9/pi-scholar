...
export function Card({ className, ...props }: HTMLAttributes<HTMLElement>) {
  return <section className={cx("rounded-lg border border-line bg-paper p-5 shadow-quiet sm:p-6", className)} {...props} />;
}

export function Badge({ children, tone = "neutral" }: { children: ReactNode; tone?: "neutral" | "positive" | "caution" | "danger" }) {
  const styles = {
  ...
  return <span className={cx("inline-flex rounded-full border px-2.5 py-1 text-xs font-bold", styles)}>{children}</span>;
}
...
export function StateView({ title, children, tone = "neutral" }: { title: string; children: ReactNode; tone?: "neutral" | "danger" }) {
  return (
    <div className={cx("rounded-lg border bg-paper p-6", tone === "danger" ? "border-danger/40" : "border-line")} role={tone === "danger" ? "alert" : "status"}>
