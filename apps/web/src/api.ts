...
function isEnum(value: unknown, values: readonly string[]): boolean { return typeof value === "string" && values.includes(value); }
...
function isQuestion(value: unknown): value is PublicQuizQuestionRecord {
  return isRecord(value)
    && Object.keys(value).every((field) => ["questionId", "quizId", "ordinal", "kind", "prompt", "choices", "cardIds", "sourceRefs"].includes(field))
...
function isLearning(value: unknown): value is WikiPageResult["learning"] {
  ...
  return ["cards", "bindings", "prerequisites"].every((field) => isRecordArray(value[field]))
    && Array.isArray(value.lineage)
    && value.lineage.every((item) => isRecord(item) && isStringArray(item.parentCardIds) && isStringArray(item.childCardIds));
}
...
function isSource(value: unknown): value is PublicSourceRecord {
  return isRecord(value)
    && Object.keys(value).every((field) => ["sourceId", "kind", "status", "displayName", "originalName", "sourceUri", "mediaType", "repositoryRevision", "capturedAt", "digest", "errorCode", "errorMessage", "createdAt", "updatedAt"].includes(field))
...
function isPage(value: unknown): boolean {
  return isRecord(value) && hasStrings(value, ["pageId", "relativePath", "title", "digest", "status", "quizWorthiness", "updatedAt"]) && isEnum(value.status, PAGE_STATUSES) && isEnum(value.quizWorthiness, QUIZ_WORTHINESS) && typeof value.revision === "number";
}

function isQuiz(value: unknown): value is PublicQuizRecord {
  return isRecord(value)
    && Object.keys(value).every((field) => ["quizId", "date", "revision", "status", "questions", "answers", "draft", "questionResults", "cardResults", "grades", "readings", "generatedAt", "submittedAt"].includes(field))
    ...
    && ["questionResults", "cardResults", "grades"].every((field) => value[field] === undefined || isRecordArray(value[field]))
...
function isWorkflow(value: unknown): boolean {
  return isRecord(value) && hasStrings(value, ["requestId", "kind", "status"]) && isEnum(value.kind, WORKFLOW_KINDS) && isEnum(value.status, WORKFLOW_STATUSES) && typeof value.progress === "number";
}

export const isHealthResult: ResultGuard<HealthResult> = (value): value is HealthResult =>
  isRecord(value) && hasStrings(value, ["status", "version"]) && ["ok", "degraded", "failed"].includes(String(value.status));
...
export const isSourceRemovalPreviewResult: ResultGuard<SourceRemovalPreviewResult> = (value): value is SourceRemovalPreviewResult =>
  isRecord(value) && isSource(value.source) && typeof value.confirmationId === "string" && isStringArray(value.dependentPageIds) && isStringArray(value.dependentCardIds);

export const isSourceRemovalResult: ResultGuard<SourceRemovalResult> = (value): value is SourceRemovalResult =>
  isRecord(value) && hasStrings(value, ["sourceId", "status"]) && value.status === "removed" && isStringArray(value.dependentPageIds) && isStringArray(value.dependentCardIds);
...
export const isWikiPageResult: ResultGuard<WikiPageResult> = (value): value is WikiPageResult =>
  ...
  && value.sections.every((section) => isRecord(section) && typeof section.anchor === "string" && (section.heading === undefined || typeof section.heading === "string"))
  && isLearning(value.learning)
  && (value.drift === undefined || (isRecord(value.drift) && hasStrings(value.drift, ["expectedDigest", "actualDigest", "diff"])));

export const isWikiIssueListResult: ResultGuard<WikiIssueListResult> = (value): value is WikiIssueListResult =>
  isRecord(value) && Array.isArray(value.issues) && value.issues.every((issue) => isRecord(issue) && hasStrings(issue, ["issueId", "kind", "description", "status", "createdAt", "updatedAt"]) && isEnum(issue.kind, ISSUE_KINDS) && isEnum(issue.status, ISSUE_STATUSES));
...
export const isQuizResult: ResultGuard<QuizResult> = (value): value is QuizResult =>
  ...
  && ["available", "submitted", "expired", "skipped", "failed", "not-yet-run", "maintenance-day"].includes(value.outcome)
  && Array.isArray(value.answers)
  && value.answers.every(isAnswer)
  && isRecordArray(value.grades)
  && Array.isArray(value.readings)
...
export const isQuizAnswersResult: ResultGuard<QuizAnswersResult> = (value): value is QuizAnswersResult =>
  isRecord(value) && typeof value.revision === "number" && typeof value.savedAt === "string" && Array.isArray(value.answers) && value.answers.every(isAnswer);

export const isQuizSubmissionResult: ResultGuard<QuizSubmissionResult> = (value): value is QuizSubmissionResult =>
  isRecord(value) && value.status === "sealed" && isWorkflow(value.workflow) && isQuizDetail(value.quiz) && isRecordArray(value.grades) && Array.isArray(value.readings) && value.readings.every(isReading);
...
export async function api<T>(path: `/api/v1/${string}` | "/healthz", init?: RequestInit, guard?: ResultGuard<T>): Promise<T> {
  ...
  if (!isRecord(envelope) || typeof envelope.ok !== "boolean" || (envelope.requestId !== undefined && typeof envelope.requestId !== "string")) {
  ...
  if (!response.ok) throw new ApiRequestError(`Request failed with status ${response.status}`, "http-error", response.status);
...
export function formatDate(value: string, options?: Intl.DateTimeFormatOptions): string {
  ...
  return Number.isNaN(date.valueOf()) ? value : new Intl.DateTimeFormat(undefined, options ?? { dateStyle: "medium" }).format(date);
}
