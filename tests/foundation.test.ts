...
import { qmdArgs, qmdCollection, qmdDependencyIdentity, qmdEnvironment, qmdScopeCheck, qmdSearch } from "../src/external/qmd.js";
import { runChildSync } from "../src/external/process.js";
import { acquireWriterLock, initVault, readFileNoFollow, resolveVault, safeRelativePath, type VaultPaths } from "../src/vault.js";
...
describe("vault foundation", () => {
  ...
  it("checks qmd collection metadata against the physical wiki root", () => {
    ...
    const result = qmdScopeCheck(paths, (_paths, args) => {
      ...
      return { executable: "qmd", args: [...args], code: 0, signal: null, timedOut: false, stdout: `  Path:     ${collection.root}\n  Pattern:  ${collection.include}\n`, stderr: "" };
    });
    assert.equal(result.ok, true);
    const mismatch = qmdScopeCheck(paths, (_paths, args) => ({ executable: "qmd", args: [...args], code: 0, signal: null, timedOut: false, stdout: `Path: ${paths.sourcesRoot}\nPattern: ${collection.include}\n`, stderr: "" }));
    assert.equal(mismatch.ok, false);
    const duplicate = qmdScopeCheck(paths, (_paths, args) => ({ executable: "qmd", args: [...args], code: 0, signal: null, timedOut: false, stdout: `  Path: ${collection.root}\n  Path: ${collection.root}\n  Pattern: ${collection.include}\n`, stderr: "" }));
  ...
  it("isolates qmd configuration/cache and requests bounded JSON search output", async () => {
    const root = mkdtempSync(join(tmpdir(), "pi-scholar-"));
    const paths = initVault(join(root, "vault"));
    assert.deepEqual(qmdEnvironment(paths), { HOME: paths.qmdRoot, XDG_CACHE_HOME: join(paths.qmdRoot, "cache"), QMD_HOME: paths.qmdRoot });
    const result = await qmdSearch(paths, "hybrid query", 7, (_paths, args) => {
      assert.deepEqual(args, ["query", "hybrid query", "--format", "json", "-n", "7"]);
      return Promise.resolve({ executable: "qmd", args: [...args], code: 0, signal: null, timedOut: false, stdout: "[]", stderr: "" });
    });
  ...
  it("pins qmd commands to the vault collection and rejects overrides", () => {
    ...
    const paths = initVault(join(root, "vault"));
    const collection = qmdCollection(paths);
    assert.deepEqual(qmdArgs(paths, ["collection", "add", collection.root, "--name", collection.name, "--mask", collection.include]), ["--collection", collection.name, "collection", "add", collection.root, "--name", collection.name, "--mask", collection.include]);
    assert.deepEqual(qmdArgs(paths, ["collection", "show", collection.name]), ["--collection", collection.name, "collection", "show", collection.name]);
    assert.deepEqual(qmdArgs(paths, ["update"]), ["--collection", collection.name, "update"]);
    assert.deepEqual(qmdArgs(paths, ["query", "term", "--format", "json", "-n", "100"]), ["--collection", collection.name, "query", "term", "--format", "json", "-n", "100"]);
    for (const args of [
    ...
    ] as const) assert.throws(() => qmdArgs(paths, args));
  });

  it("fails closed on dependency identity results and isolates Docling state", () => {
    const root = mkdtempSync(join(tmpdir(), "pi-scholar-"));
    const paths = initVault(join(root, "vault"));
    assert.deepEqual(doclingEnvironment(paths), { HOME: paths.workRoot, XDG_CACHE_HOME: join(paths.workRoot, "cache"), DOCLING_CACHE_DIR: paths.workRoot });
    const qmd = qmdDependencyIdentity(paths, (_paths, args) => ({ executable: "/pinned/qmd", args: [...args], code: 0, signal: null, timedOut: false, stdout: "qmd version 2.5.3", stderr: "" }));
    assert.deepEqual(qmd, { executable: "/pinned/qmd", version: "qmd version 2.5.3" });
    const docling = doclingDependencyIdentity(paths, (_paths, args) => ({ executable: "/pinned/docling", args: [...args], code: 0, signal: null, timedOut: false, stdout: "Docling version 2.4.1", stderr: "" }));
    ...
    for (const result of [
      ...
      assert.throws(() => qmdDependencyIdentity(paths, (_paths, args) => ({ executable: "/pinned/qmd", args: [...args], ...result, stdout: "qmd version 2.5.3", stderr: "" })));
      assert.throws(() => doclingDependencyIdentity(paths, (_paths, args) => ({ executable: "/pinned/docling", args: [...args], ...result, stdout: "Docling version 2.4.1", stderr: "" })));
    }
    assert.throws(() => qmdDependencyIdentity(paths, (_paths, args) => ({ executable: "/pinned/qmd", args: [...args], code: 0, signal: null, timedOut: false, stdout: "not-a-version", stderr: "" })));
    assert.throws(() => doclingDependencyIdentity(paths, (_paths, args) => ({ executable: "/pinned/docling", args: [...args], code: 0, signal: null, timedOut: false, stdout: "", stderr: "" })));
  });
  ...
  it("doctor rejects a tampered quiz answer without rewriting the sheet", () => {
    ...
    db.run(
      ...
      ["doctor-question", "doctor-quiz", "Explain the card", JSON.stringify([{ cardId: "doctor-card", criterion: "Explain", weight: 1 }])],
    );
    db.run(
      "INSERT INTO question_cards (question_id, card_id, criterion_json, weight) VALUES (?, ?, ?, ?)",
      ["doctor-question", "doctor-card", JSON.stringify("Explain"), 1],
    );
    db.run(
      "INSERT INTO quiz_answers (quiz_id, question_id, revision, answer_json, saved_at) VALUES (?, ?, 1, ?, ?)",
      ["doctor-quiz", "doctor-question", JSON.stringify("expected"), now],
    );
  ...
  it("doctor accepts an expired quiz projection with incomplete answers", () => {
    ...
    db.run(
      "INSERT INTO quiz_questions (question_id, quiz_id, ordinal, kind, prompt, choices_json, answer_key_json, grading_criteria_json, source_refs_json) VALUES (?, ?, 0, 'short-answer', ?, NULL, NULL, ?, '[]')",
      ["expired-question", "expired-quiz", "Explain the card", JSON.stringify([{ cardId: "expired-card", criterion: "Explain", weight: 1 }])],
    );
    db.run(
      "INSERT INTO question_cards (question_id, card_id, criterion_json, weight) VALUES (?, ?, ?, ?)",
      ["expired-question", "expired-card", JSON.stringify("Explain"), 1],
    );
    const quizService = new QuizService(db, paths);
  ...
  it("doctor rejects a non-removed source row whose packet is missing", () => {
    ...
    db.run(
      ...
      ["reverse-source", "Reverse source", "reverse.txt", now, "digest", join(paths.sourcesRoot, "reverse-source"), now, now],
    );
    db.close();
    const report = doctor(paths.vaultRoot);
  ...
  it("applies the same fail-closed Git option validation to async and sync adapters", async () => {
    const root = mkdtempSync(join(tmpdir(), "pi-scholar-"));
    const paths = initVault(join(root, "vault"));
    for (const args of [["push", "--force=true"], ["push", "--force-with-lease=origin/main"]] as const) {
