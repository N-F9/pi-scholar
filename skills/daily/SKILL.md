...
- If initialization is enabled, stop after reading the context: do not generate questions, request evidence, or call `scholar_publish_daily` for either a quiz or a skip. Answer exactly `Daily quiz guarded for <date>. Expired prior quizzes: <expiredCount>. No quiz was published.`, substituting the context's values.
- Only if initialization is disabled, review all `QuizContext.candidates`, then choose a semantically varied but related subset that supports a coherent study session. The host has already filtered for active, due, prerequisite-unblocked, non-drifted pages but has not imposed a topical ordering. Size the combined reading and quiz work for 15–45 minutes, with a mental median near 30 minutes. There is no fixed question, page, synthesis, or timer cap.
...
- If initialization is disabled and no candidate exists, call `scholar_publish_daily` with the explicit skip result for today's date. Do not invent filler material.
- If initialization is disabled and candidates exist, call `scholar_publish_daily` once with today's proposal. Do not alter page learning or prerequisite state or publish a second revision. Proposals do not provide question IDs; the host mints opaque UUIDs.
